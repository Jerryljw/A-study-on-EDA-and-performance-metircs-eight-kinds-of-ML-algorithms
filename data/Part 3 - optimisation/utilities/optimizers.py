...
def mini_batch_gradient_descent(y, x, theta, max_iters, alpha, metric_type, mini_batch_size):
    ...
    for i in range(max_iters):
        ...
        x, y = x[indx], y[indx]
        for idx in range(0,num_of_samples, mini_batch_size):
            xs = x[idx:idx+mini_batch_size]
            ys = y[idx:idx+mini_batch_size]
            num_of_samples = len(xs)
            gradient = -2 * xs.T.dot(ys - xs.dot(theta)) / num_of_samples
            theta = theta - alpha * gradient
            loss = compute_loss(ys, xs, theta, metric_type)
            thetas.append(theta)
            losses.append(loss)
